...
<html lang="fr">
  ...
  <head>
    ...
    <title>Liste des Livres délivrés</title>
...
table#t01 {
  ...
  <body>
	...
	<header class="masthead" style="background-image: url('img/livredelivre.jpg')">
      <div class="overlay"></div>
      <div class="container">
        <div class="row">
          <div class="col-lg-8 col-md-10 mx-auto">
            <div class="page-heading">
              <h1>Liste des Livres Délivrées</h1>
            </div>
          </div>
        </div>
      </div>
	...
	<input type="text" name="titre" placeholder="Search By Name" value="">
...
if (count($_POST)==0)
{
$tous = $db->query('select * From livre WHERE nbreLivreDispo=0 ORDER BY titre');
$livres = $tous->fetchAll(PDO::FETCH_OBJ);
foreach ($livres as $livre) 
{
$requette=$db->query("SELECT nom_cat FROM appartient_cat WHERE ref_livre='".$livre->reference."'") ;
$categories=$requette->fetchAll(PDO::FETCH_OBJ);
echo "<tr>" ;
...
{
$param = "%{$_POST['titre']}%";
$req1="SELECT * FROM livre WHERE nbreLivreDispo=0 AND titre LIKE :titre ORDER BY titre" ;
$res2=$db->prepare($req1);
$res2->execute(array('titre'=>$param)) ;
$livres = $res2->fetchAll(PDO::FETCH_OBJ);
foreach ($livres as $livre) 
{echo "<tr>" ;
echo "<td>".$livre->reference."</td>";
echo "<td>".$livre->titre."</td>";
